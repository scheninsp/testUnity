...
float2 ParallaxOffset (float2 uv, float2 viewDir) {
	//简单版本的 Parallax mapping， offset 依赖于 viewDir的投影长度，以及
	// 采样与 Geometry 平面交点得到的 uv 时，从 parallax map 获取的高度值
...
float2 ParallaxRaymarching (float2 uv, float2 viewDir) {
	...
	for (
		...
		// stepHeight 低于 surfaceHeight代表采样已经穿透了高度面
		i++
	) {
